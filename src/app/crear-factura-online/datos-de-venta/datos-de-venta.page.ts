...
export class DatosDeVentaPage implements OnInit {
  ...
  ionViewWillLeave(){
    ...
    if(this.router.url != "/datos-de-cliente"){
      if(!this.dataFacturaServ.dataFactura.isProcesadaInterno){
  ...
  ngOnDestroy(){
    ...
    if(!this.dataFacturaServ.dataFacturaTemporalCopy.isProcesadaInterno){
      this.eliminarFactura();
    }
  ...
  eliminarFactura(){
    ...
    this.toastServ.toastMensajeDelServidor("Eliminando factura... " , "success",1000)
